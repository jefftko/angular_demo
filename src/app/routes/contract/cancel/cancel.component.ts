...
export class CancelComponent implements OnInit {
    ...
    searchFields:any = [
        ...
        {'field':'contractStatus','title':'合同状态','type':'1','options':[{'value':'YSX','label':'已生效'},{'value':'YZZ','label':'已中止'}]},
    ...
    {'field':'cancelDate','title':'合同中止日期','template':'date','disabled':'YZZ'},
    {'field':'resumeDate','title':'合同恢复日期','template':'date','disabled':'YSX'},
    ...
    {'field':'resumeStatusDesc','title':'恢复审批状态','template':'text'},
...
attachListConfig:any = {
    ...
    getOrgId(){
        ...
        if(orgId != undefined){
            this.searchFields[4]['options'] = JSON.parse(orgId);
           ...
           this.contractService._get(APIS.SEARCH.getOrgId).then((data:any)=>{
            ...
            if(row.length != 0){
                localStorage.setItem('orgId',JSON.stringify(row))
    ...
    getData(data){
        ...
        this.contractService._post(APIS.CONTRACT.query+'/cancel',data)
        .then((data: any) => {
    ...
    if(type == 'YZZ' && (this.selectedItem.cancelDate == '' || this.selectedItem.cancelDate == undefined)){
         this.msg.create('warning','合同中止日期不能为空')
        return
    }
    if(type == 'YSX' && (this.selectedItem.resumeDate == ''||this.selectedItem.resumeDate == undefined)){
        this.msg.create('warning','合同恢复日期不能为空')
        return
    }
      //this.selectedItem.contractStatus = type;
       //this.selectedItem.contractStatusDesc = this.contractStatusDesc[type];
    this.contractService._post(APIS.CONTRACT.cancel,this.selectedItem)
        .then((data: any) => {
            console.log(data)
            if(data.errcode == 0)
